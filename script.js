...
document
  .querySelector(".grid-select")
  .addEventListener("change", changeLayoutHandler);
...
const addElementHandler = function (evt) {
  ...
  const templateElement = document
    .querySelector("#" + blockType + "-template")
    .content.cloneNode(true);
  const blockElement = templateElement.querySelector(".element");
  //console.log(blockElement);

  // вставить элемент в свой контейнер
  const containerWrapperElement = document.querySelector(
    "." + blockContainer + "__elements-wrapper"
  );
  ...
  if (blockContainer.includes("content")) {
    ...
    containerWrapperElement.parentElement.classList.remove(
  ...
  blockElement
    ...
    .addEventListener("click", buttonDeleteHandler);

  // ********************* урок 14  *********************
  blockElement
    .querySelector(".template-content")
    .addEventListener("dblclick", editContentHandler); // ,true ????
};
